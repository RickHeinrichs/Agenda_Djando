...
def lista_eventos(request):
    ...
    evento = Evento.objects.filter(usuario=usuario,
                                   data_evento__gt=data_atual)   # (__lt mostra menores) __gt Mostra apenas eventos = ou a frente da data atual#Todos os ID      #get(id=1) Traz apenas um ID escolhido
...
def json_lista_evento(request, id_usuario):
    ...
    return JsonResponse(list(evento), safe=False) #list converte em lista